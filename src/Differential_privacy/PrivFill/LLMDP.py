...
from transformers import AutoModelForSeq2SeqLM, AutoTokenizer, AutoModelForCausalLM, LogitsProcessor, LogitsProcessorList
...
class DPPrompt():
    ...
    def __init__(self, model_checkpoint="google/flan-t5-large", min_logit=-95, max_logit=8, batch_size=16):
        ...
        # Use model directly instead of pipeline; "text2text-generation" was removed in newer transformers
    ...
    def privatize_dp(self, texts, epsilon=100, max_new_tokens=32):
        """Privatize a list of texts (one sentence at a time; batching is slower on CPU/MPS)."""
        temperature = 2 * self.sensitivity / epsilon
        private_texts = []
        for text in texts:
            prompt = self.prompt_template_fn(text)
            model_inputs = self.tokenizer(prompt, max_length=512, truncation=True, return_tensors="pt").to(self.device)
            max_len = min(max_new_tokens, model_inputs["input_ids"].shape[1])
            output = self.model.generate(
                **model_inputs,
                do_sample=True,
                top_k=0,
            ...
            private_texts.append(self.tokenizer.decode(output[0], skip_special_tokens=True))
