...
class Lists():
    ...
    # Update file loading for vocabulary
    with open(os.path.join(_current_dir, "data", "vocab.txt"), 'r') as f:
    ...
    def __init__(self, home, algorithm=Algorithm.Flat, num_lists=1, return_list=True, save_directory=None, vocab=None, model_names=None):
        ...
        # Check if cached lists exist
        cache_path = os.path.join(self.home, f"cached_lists_{algorithm.name}_{num_lists}.pkl")
        if os.path.exists(cache_path):
            print("Loading lists from cache...")
            with open(cache_path, 'rb') as f:
                self.lists = pickle.load(f)
            print("Cached lists loaded successfully.")
        else:
            self.lists = self.get_all_lists()
            # Save to cache for future use
            print("Saving lists to cache...")
    ...
    def create_mappings(self):
        ...
        print("Word mappings created successfully")
        return
    ...
    def get_vocabulary(self):
        """Return the vocabulary as a list of words."""
        return list(self.mapping_word.keys())
    ################################################################
...
class Diffractor():
    ...
    def rewrite(self, texts, epsilon=None):
        ...
        for i, s in enumerate(tqdm(texts, disable=self.progress_bar)):
            ...
            temp_replace = []
            text_changes = []  # Track changes for this text
            ...
            for i in range(len(cands[0])):
                ...
                if len(i_cands) == 0:
                    ...
                    if tokens[i] != new_word:
                        num_diff += 1
                        text_changes.append((tokens[i], new_word))  # Record change
                support.append(len(set(i_cands)))
                total += 1
                temp_replace.append(new_word)

            perturbed.append(" ".join(temp_replace))
            changes.append(text_changes)  # Add changes for this text

        return perturbed, num_perturbed, num_diff, total, round(np.mean(support), 3), changes
